...
%---------------------------------------------------
rep_r_scan_dist_1 = repmat(r_scan_dist_1, [1, nparticles]);
...
%     index_min_err_shift = (index_min_err_shift - 1) * (360/nscans);
    index_min_err_shift = (mod(index_min_err_shift - 2, nscans)) * (360/nscans);
    ...
    dir = [cos(ang) sin(ang)];
else%if((stdX < 15 && stdY < 15 && stdX > 7 && stdY > 7) || last_shift == true)
%     index_min_err_shift = (index_min_err_shift - 1) * (360/nscans);
    index_min_err_shift = (mod(index_min_err_shift - 1, nscans)) * (360/nscans);
    index_min_err_shift = degtorad(index_min_err_shift);
    
%     ang = bsxfun(@minus, ang, bsxfun(@or, bsxfun(@times, index_min_err_shift, index_min_err_shift~=(round(nscans/2)-1)), ...
%                               bsxfun(@or, ...
%                               bsxfun(@times, index_min_err_shift, index_min_err_shift~=round(nscans/2)), bsxfun(@times, index_min_err_shift, index_min_err_shift~=(round(nscans/2)+1)))));
