...
rep_inter = intersectionQ(rep_scan, rep_map);

botpos_mat = repmat(botpos, rep_dim, 1);
diff = bsxfun(@minus, rep_inter, botpos_mat);
distNN = bsxfun(@hypot, diff(:, 1), diff(:, 2)) + bsxfun(@times,bsxfun(@times, sign(ones(size(diff, 1),1)-0.5), rand(size(diff, 1), 1)), rand(size(diff, 1), 1).*10);
