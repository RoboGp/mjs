...
for ind = 1:nparticles
    ...
    movdist = mlist(1, 2);
    inside = checkPoint((del_ind - 1) * turn_angle, botang, botpos, movdist, pad_map_lines, pad_inpolygonMapformatX, pad_inpolygonMapformatY);
%     [ang(ind) dir(ind, :)] = turn(ang(ind), delta_angle, dir(ind, :), turningNoise);
    [ang(ind) dir(ind, :)] = turn(ang(ind), (del_ind - 1) * turn_angle, dir(ind, :), turningNoise);
