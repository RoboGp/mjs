...
disp('You can set the angle and position of the robot with:');
input('setBotPos() and setBotAng(), press enter to initialise the robot');
botSim.setBotPos([20 40])
botSim.setBotAng(pi)

disp(' ');
disp('You can get the current bot position with getBotPos()');
...
disp('You can set the motion noise level with the variable motionNoise');
...
disp(' ');
disp('You can also set the angular noise with the variable turningNoise');
input('Press enter to show the robot move from one location 50 times');
hold off
botSim.setBotPos([25 10]);
botSim.setBotAng(pi/2);
